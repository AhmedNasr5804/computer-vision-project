...
The dataset ships with binary lane masks regenerated by
SegFormer-B2-cityscapes \cite{segformer}, but these are unusable
for supervision on this domain (Section~\ref{sec:lane_rebuild}):
the rig shows a dark road on cream fabric rather than a real road
scene, so the Cityscapes-trained model returns near-empty masks on
most frames (median $0.8\%$ nonzero, range $0.0$--$13.6\%$). We
replace them with masks regenerated by a four-step classical
pipeline (grayscale~$\rightarrow$~Gaussian blur~$k\!=\!5$
$\rightarrow$~Otsu$+$INV threshold $\rightarrow$~$3\!\times\!3$
morphological opening $\rightarrow$~drop connected components
smaller than $200$\,px). The regenerated masks have a median
coverage of $34.2\%$ and visually match the dark road region in
every frame (Fig.~\ref{fig:lanemasksv2}).

\textbf{Phone fine-tune (eye).} 44 full-frame portraits captured on
the S24~Ultra (22 open / 22 closed, six subjects). The displayed
open / closed pair in Fig.~\ref{fig:phonedetect} is drawn from a
second capture session that the author has explicitly cleared for
inclusion; the original 12 captures from an earlier session involve
other people and are used internally by the fine-tune script but not
reproduced in this paper for privacy. All captures are
full-face photographs in landscape orientation, so the fine-tune
pipeline first rotates them to portrait via a four-rotation
face-detection vote, then crops the detected face to
$100\!\times\!100$ to match the CEW domain before any fine-tune step.
...
one subject, drawn from the 32 newly-captured photos in the second
fine-tune session, after orientation correction and Haar face
detection. The pipeline correctly locates the largest face in both
frames despite landscape capture; the same behavior holds for the
remaining 30 captures in this session and the 12 from the prior
session, which are omitted from this figure for privacy.}
...
\begin{figure}[t]
\centering
\subfloat[Exp-A augmentation]{\includegraphics[width=0.32\columnwidth]{fig_eye_expA.png}}\hfill
\subfloat[Exp-B color space]{\includegraphics[width=0.32\columnwidth]{fig_eye_expB.png}}\hfill
\subfloat[Exp-D lightweight]{\includegraphics[width=0.32\columnwidth]{fig_eye_expD.png}}
...
\begin{figure}[t]
\centering
\subfloat[Exp-A augmentation (lane)]{\includegraphics[width=0.48\columnwidth]{fig_lane_expA.png}}\hfill
\subfloat[Exp-D lightweight (lane)]{\includegraphics[width=0.48\columnwidth]{fig_lane_expD.png}}
...
\subsection{Pi-domain pipeline rebuild}\label{sec:lane_rebuild}

Initial attempts to fine-tune the TuSimple-trained \textit{lw\_baseline}
on the Pi domain produced a misleading $1.000$ accuracy on the Pi
test set against a near-empty validation signal. Diagnosis revealed
two problems compounding: (i) the SegFormer-Cityscapes road masks
shipped with the Pi dataset are empty or noise on most frames
because the rig is not a real road scene, and (ii) a $\sim 90$\,k
parameter TuSimple-trained encoder is unnecessary capacity for a
test-bench of high-contrast tape on cream fabric. After review with
the course supervisor we restructured the Pi-side pipeline along
four corrective points and report the resulting numbers below.

\textbf{Step~1: sanity overfit (data audit).}
A tiny U-Net of $7{,}193$ parameters was trained for 60 epochs on a
fixed subset of 50 Pi images, with no augmentation and no
regularization, to test whether the masks have any learnable signal.
Training the same model with the \emph{SegFormer} masks plateaued
at $\text{IoU}=0.244$ (Fig.~\ref{fig:lanesanity}, left)---a
$7{,}193$-parameter model cannot overfit 50 images of garbage.
Re-running with the regenerated \emph{Otsu+morphology} masks
reached $\text{IoU}=0.992$ in the same 60 epochs
(Fig.~\ref{fig:lanesanity}, right). The data-quality verdict is
binary: SegFormer's output on this domain is unusable; the
classically-regenerated output is faithful.

\begin{figure}[t]
\centering
\subfloat[On SegFormer masks: IoU plateaus at $0.244$]
  {\includegraphics[width=0.48\columnwidth]{fig_lane_phase1a_overfit.png}}\hfill
\subfloat[On Otsu masks: IoU climbs to $0.992$]
  {\includegraphics[width=0.48\columnwidth]{fig_lane_phase1b_overfit.png}}
\caption{Sanity overfit of a $7{,}193$-parameter U-Net on 50 Pi
images for 60 epochs. Identical model and optimizer; the only thing
that changes is the supervision signal. The diagnosis is unambiguous:
the SegFormer-Cityscapes masks have no learnable signal on the
test-bench Pi domain.}
\label{fig:lanesanity}
\end{figure}

\textbf{Step~2: mask regeneration via classical CV.}
We replace the SegFormer masks with the Otsu pipeline described in
Section~\ref{sec:method} on every one of the 10{,}500 frames
(20.8\,ms/image on CPU, 218\,s total).
Fig.~\ref{fig:lanemasksv2} compares both mask sources on four random
frames (one per direction, plus an augmented copy). The
median fraction of lane pixels rises from $0.8\%$ (SegFormer) to
$34.2\%$ (Otsu) and visually traces the dark road in every frame.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_lane_masks_v2.png}
\caption{Mask source comparison on four random Pi frames. Columns
left to right: input image, SegFormer-Cityscapes mask (largely
empty), regenerated Otsu mask, overlay of the new mask on the input.
The new masks correctly trace the dark road on every frame.}
\label{fig:lanemasksv2}
\end{figure}

\textbf{Step~3: truncated U-Net architecture.}
We then build a deliberately shallow segmentation network: two
encoder blocks ($\text{Conv}(8)\!\rightarrow\!\text{Conv}(16)$), a
24-filter bottleneck, two decoder blocks mirroring the encoder, and
a final $1\!\times\!1$ sigmoid head, for $12{,}609$ parameters
total. The rationale---per the doctor's direct guidance---is that
the Pi domain has no high-level features that benefit from depth:
the model only needs to discriminate dark-road pixels from
cream-fabric pixels, and two pooling stages of $128\!\times\!128$
input give a $32\!\times\!32$ receptive field that already covers
the largest lane curvature in the dataset. The earlier
TuSimple-trained \textit{lw\_baseline} at $\sim 86$\,k parameters is
$6.8\times$ larger and brings no benefit on this domain.

\textbf{Step~4: train first, quantize second.}
Following the supervisor's instruction to ``train before
optimization to preserve features,'' we train the truncated U-Net
in FP32 (Adam~$10^{-3}$, $32$-batch, EarlyStopping on val IoU with
patience~$6$) until it converges, then apply post-training
quantization---never QAT during training, never quantize the
untrained network. Training runs for 25 epochs in $9.8$\,min on
desktop CPU.

Table~\ref{tab:lane_rebuild} reports the final numbers. The FP32
truncated U-Net reaches $\text{IoU}=0.9969$ on the test split
(1{,}583 images, source-frame-stratified split with no augmented
copies leaking across splits), and PTQ-int8 quantization with a
200-image representative dataset retains IoU to within
$0.003$\,pp---essentially a free $2.25\times$ compression that
brings the deployment artefact down to $26.7$\,KB.
Fig.~\ref{fig:lanephase4train} shows training and validation
curves; Fig.~\ref{fig:lanephase4preds} shows eight random test
predictions, in which the lane outline is recovered to
single-pixel accuracy.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_lane_phase4_train.png}
\caption{Truncated U-Net training: BCE$+$Dice loss, IoU, and Dice
on train and validation across 25 epochs. The model converges to
$\text{val IoU}=0.997$ within 13 epochs and EarlyStopping fires
once the metric plateaus.}
\label{fig:lanephase4train}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.75\columnwidth]{fig_lane_phase4_preds.png}
\caption{Eight random test-set predictions of the truncated U-Net.
Columns: input, ground-truth Otsu mask, predicted lane mask. The
predicted boundary is pixel-tight with the ground truth on every
sample.}
\label{fig:lanephase4preds}
\end{figure}

\begin{table}[t]
\centering
\caption{Truncated U-Net on the Pi-camera test split (1{,}583
source-frame-stratified images) after the four-step rebuild.}
\label{tab:lane_rebuild}
\begin{tabular}{lcccr}
\toprule
Variant & IoU & Dice & Size & $\Delta$ IoU \\
\midrule
FP32 TFLite          & 0.9969 & 0.9985 & 60.0\,KB & ---     \\
PTQ-int8 TFLite      & 0.9969 & 0.9984 & \textbf{26.7\,KB} & $-0.003$\,pp \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Three-class extension}\label{sec:results_3class}

After the binary segmentation passed the supervisor's data-and-model
audit, we extend the head from $\text{Conv}(1,\sigma)$ to
$\text{Conv}(3,\mathrm{softmax})$ and split the binary lane region
at its median-x coordinate: pixels left of the lane centroid become
class~1 (\textit{lane-left}), pixels right become class~2
(\textit{lane-right}), background remains class~0. This bisection
gives the model a steering-relevant left/right partition while
leaving the backbone unchanged. Fig.~\ref{fig:lane3classmask} shows
the resulting three-class masks on the same four representative
frames from Fig.~\ref{fig:lanemasksv2}: the lane region is split
into two roughly equal halves whose boundary is the median-x of
the lane region in that frame.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_lane_3class_mask_preview.png}
\caption{Three-class masks derived from the binary masks by
median-x bisection of the lane region. Background is grey, the
left half of the lane is red, the right half is green. The
bisecting axis tracks the lane's geometric center, so it shifts
with the lane's position in the frame---directly relevant to a
downstream steering decision.}
\label{fig:lane3classmask}
\end{figure}

The truncated U-Net trained on the three-class masks for 30 epochs
($12{,}627$ parameters; the head adds 9 parameters and 9 biases
relative to the binary head, see Table~\ref{tab:lane_rebuild_3c}).
Test IoU on the source-frame-stratified split reaches
$0.9984$ (background), $0.9907$ (lane-left), $0.9903$ (lane-right)
---mean $0.9931$, with both lane classes comfortably above the
$0.6$ gate set in advance. PTQ-int8 of the three-class model
ships at $27.0$\,KB (within 0.3\,KB of the binary model, since the
softmax head adds only nine parameters). Fig.~\ref{fig:lanephase6train}
plots the training curves; Fig.~\ref{fig:lanephase6preds} shows
eight random test predictions where the predicted three-class map
matches the ground-truth median-x partition with no visible defect.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_lane_phase6_train.png}
\caption{Three-class truncated U-Net training: CCE+Dice loss and
per-class IoU on the validation set. Lane-left and lane-right
(both initialized far below background due to class imbalance,
$\sim$16\% each vs $\sim$67\% background) catch up within three
epochs and stabilize above $0.99$ by epoch 20.}
\label{fig:lanephase6train}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.75\columnwidth]{fig_lane_phase6_preds.png}
\caption{Eight random three-class test-set predictions. Columns:
input, ground-truth three-class mask (0\,=\,background,
1\,=\,lane-left, 2\,=\,lane-right), predicted three-class mask.
The bisecting axis is reproduced with no visible offset.}
\label{fig:lanephase6preds}
\end{figure}

\begin{table}[t]
\centering
\caption{Three-class truncated U-Net on the Pi-camera test split
(1{,}583 source-frame-stratified images).}
\label{tab:lane_rebuild_3c}
\begin{tabular}{lcccc}
\toprule
Variant & IoU bg & IoU L & IoU R & Size \\
\midrule
FP32 TFLite         & 0.9984 & 0.9907 & 0.9903 & 60.4\,KB \\
PTQ-int8 TFLite     & 0.9984 & 0.9907 & 0.9903 & \textbf{27.0\,KB} \\
\bottomrule
\end{tabular}
\end{table}
...
On the 44 S24-captured photos used in Section~\ref{sec:results}
(six subjects, 22 open / 22 closed), the CEW-trained winner reaches
only $0.568$ accuracy zero-shot ($0.5625$ on the 32-photo subset
displayed in Fig.~\ref{fig:appft}), with the largest errors on
oblique gazes and subjects with beards or visible hair---two
demographics that are under-represented in CEW. A short fine-tune on
the same 44 photos (20 epochs, batch 4, Adam,
lr=$1\!\times\!10^{-4}$, $\approx 4.2$\,s on a desktop CPU) brings
per-subject accuracy to $1.000$ (blue bars), at the cost of
$14.3$\,pp on the canonical CEW test split
($0.907 \rightarrow 0.764$). PTQ-int8 conversion of the fine-tuned
model retains $0.764$ CEW accuracy and the full $1.000$ on the 44
fine-tune photos, and shrinks the deployment model from 383\,KB to
107\,KB (3.6$\times$ smaller). This is the \textit{deployment} model
shipped inside the APK; the headline $0.907$ accuracy in
Table~\ref{tab:eye} refers to the original unfine-tuned winner
reported for the canonical benchmark.
...
\caption{Per-sample $P(\mathrm{open})$ on the 32 newly-captured
S24-Ultra fine-tune photos (the additional 12 photos from a prior
session are used for training but omitted here for privacy), before
(grey) and after (blue) the 20-epoch personal fine-tune. Truth
markers above each bar pair: green circle = open, red square =
closed. The base CEW-trained winner decides $P(\mathrm{open})<0.5$
on roughly half the open faces (out-of-distribution); the
fine-tuned model is correct on all 32 displayed photos (and on all
44 used for training) with high confidence.}
\label{fig:appft}
\end{figure*}
...
\textbf{Why the original Pi accuracy was suspect, and how the
rebuild fixes it (lane).}
The initial $1.000$ Pi-camera test accuracy attainable by
fine-tuning the TuSimple-trained \textit{lw\_baseline} on the
Pi-camera split was an artefact of two problems compounding: (i)
each direction class in the Pi set was extracted from one source
video and then augmented to 3{,}500 frames, so the test subset
shares low-level texture statistics with the train subset, and
(ii) the supervision signal was the SegFormer-Cityscapes mask,
which contains no learnable signal on this domain---we verified
this by failing to overfit a $7$\,k-parameter U-Net on 50 frames
(Fig.~\ref{fig:lanesanity}~(a)). The rebuild documented in
Section~\ref{sec:lane_rebuild} addresses both: a stratified
\emph{source-frame} split with no augmented-copy leakage, plus
regenerated supervision masks. The truncated U-Net's
$0.9969$ IoU on the source-frame-stratified test split is therefore
a real-data number, not an artefact---and it sits on top of a
trained-from-scratch $12{,}609$-parameter network that consumed
zero TuSimple data, so no zero-shot generalization claim is
implied either way.

\textbf{CEW does not transfer zero-shot, and the fix is cheap.}
The most actionable single finding of this project is the
$0.907 \rightarrow 0.568$ accuracy drop the CEW-trained winner
...
recovery prescription is unspectacular: 44 labeled photos of the
eventual users (22 open / 22 closed, six subjects), four seconds
of desktop-CPU fine-tune, no architectural change. PTQ-int8 of
the resulting model is the artifact actually shipped inside the
APK at 107\,KB, with on-device CPU latency unchanged
from the unfine-tuned int8 baseline at $0.126$\,ms. We emphasize
that this is the kind of domain gap a literature-reported
$\geq 95\%$ on CEW does \emph{not} forewarn you of, and that the
per-user fine-tune is a useful template for any
drowsiness-detection product targeting non-East-Asian or
non-clean-shaven drivers.
...
\textbf{Mask regeneration is mandatory; SegFormer is a wrong tool
for this domain (lane).}
The original deployment plan trusted the SegFormer-B2 masks
\cite{segformer} that ship with the Pi fine-tune set as the
supervision signal. They are unusable: visible as sparse noise in
the bottom-right panel of Fig.~\ref{fig:data} and quantified in
Fig.~\ref{fig:lanemasksv2}. The Cityscapes-trained model has no
prior on a dark-tape-on-fabric test-bench. The replacement pipeline
is intentionally classical---grayscale, Otsu, $3\!\times\!3$
morphological open, $200$-pixel connected-component filter---and it
runs in $\sim$\,21\,ms per frame. The lesson generalizes: when a
deep segmentor's training distribution is far from your deployment
distribution, a four-line OpenCV script can deliver mask quality
that no amount of fine-tuning will recover.

\textbf{Why depth hurts on a high-contrast domain (lane).}
The $86$\,k-parameter TuSimple-trained \textit{lw\_baseline} (four
encoder blocks, three pooling stages, $\sim$31$\times$31 effective
receptive field on $128\!\times\!128$ input) brings no benefit on
the Pi domain compared with the $12{,}609$-parameter truncated
U-Net (two encoder blocks, two pooling stages). Both reach
identical test IoU after fine-tune, but the truncated model
trains in $9.8$\,min on CPU from scratch (no transfer-learning
warm start needed), occupies $26.7$\,KB after PTQ-int8, and has no
spurious sky/horizon/vehicle features inherited from TuSimple to
forget. This is consistent with the doctor's direct guidance that
\emph{``you can't fine-tune a deep model trained on high-level
features such as the TuSimple dataset with a trivial case such as
the Pi camera dataset and expect results---you should first drop
some of the high-level feature extractors.''} On a domain whose
information content collapses to a single dark/light threshold,
...
model, after a supervisor-driven rebuild of its Pi pipeline
(Section~\ref{sec:lane_rebuild})---classical mask regeneration,
architectural truncation, and a sanity overfit data audit---
reaches $\text{IoU}=0.9969$ on the source-frame-stratified test
split at only $12{,}609$ parameters and $26.7$\,KB after PTQ-int8.
The deployment process surfaced three practical findings worth
reporting on their own: (i) the literature-canonical CEW accuracy
does not transfer zero-shot to a contemporary front-camera
deployment ($-34$\,pp on 44 captured photos), but a 44-photo
personal fine-tune closes the deployment-side gap (44/44 correct)
while keeping a usable $0.764$ on the canonical CEW test split;
(ii) a Cityscapes-trained segmentor is the wrong tool for an
indoor tape-on-fabric test-bench, and a four-line classical
pipeline replaces it at $21$\,ms/frame on CPU; (iii) on a domain
without high-level features, a $12{,}609$-parameter from-scratch
truncated U-Net matches the IoU of a $\sim$\,86\,k-parameter
TuSimple-trained network while training in $9.8$\,min. PTQ-int8
of all shipped models retains accuracy within sub-pp deltas at
2--4$\times$ size compression.
